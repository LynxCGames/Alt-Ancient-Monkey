...
public class GodlyUpgrade : BloonsTD6Mod
{
    public static void UpgradeTower()
    {
        ...
        luckPanel.GetComponentFromChildrenByName<ModHelperPanel>("luckScoreBar").SetInfo(new Info("luckScoreBar", 15 * mod.luck, 75, 30 * mod.luck, 150, new Vector2()));
        ...
        // Removes all non-gilded Common weapons
        foreach (var tower in InGame.instance.GetTowers())
        {
            if (tower.towerModel.name.Contains("AncientMonkey-AncientMonkey"))
            {
                ...
                {
                    if (weapon.name.Contains("Common"))
                    {
    ...
    public static void ClassicReset()
    {
        foreach (var rarity in ModContent.GetContent<WeaponRarityTemplate>())
        {
            ...
            else if (rarity.WeaponRarity == WeaponRarityTemplate.Rarity.Rare)
            {
                rarity.minValue = 0;
                rarity.maxValue = 100;
            }
        ...
        foreach (var rarity in ModContent.GetContent<AbilityRarityTemplate>())
        {
            ...
            else if (rarity.AbilityRarity == AbilityRarityTemplate.Rarity.Rare)
            {
                rarity.minValue = 0;
                rarity.maxValue = 100;
            }
        ...
        foreach (var rarity in ModContent.GetContent<StrengthRarityTemplate>())
        {
            ...
            else if (rarity.StrengthRarity == StrengthRarityTemplate.Rarity.Rare)
            {
                rarity.minValue = 0;
                rarity.maxValue = 100;
            }
